...
def official():
    path = (
        ...
        / "NUFILT/fusion_bench/models/filter_lora.py"
    )
    if not path.exists():
...
@pytest.mark.parametrize(
    "null_space,lora_r,null_r,grad_r",
    [
        (True, 2, 2, 2),
        (False, 2, 2, 2),
        (True, 0, 2, 2),
...
def test_official_continual_result(official, shape, null_space, lora_r, null_r, grad_r):
    ...
    for i, task in enumerate(inputs[1:]):
        ...
        seed = int.from_bytes(
            hashlib.sha256(f"42:{name}:{i}".encode()).digest()[:8], "little"
        )
        with torch.no_grad():
            module.gate.A.normal_(
                std=0.02, generator=torch.Generator().manual_seed(seed)
            )
        if i > 0 and lora_r > 0:
    ...
    with torch.no_grad():
        result = merge_tensors(
            inputs, "nufilt", base_index=0, name=name, parameters=params
        )
...
def test_selection_and_rng():
    torch.manual_seed(9)
    base = {
        "encoder.attn.q.weight": torch.randn(6, 6),
    ...
    assert not torch.equal(
        output["encoder.attn.q.weight"], base["encoder.attn.q.weight"]
    )
...
def test_example_parameters_resolve():
    ...
    for name, value in config.parameters.items():
        assert (
            evaluate_setting("attn.weight", value, validate=specs[name].validate)
            == value
        )


@pytest.mark.parametrize(
    "parameter,value", [("lr", -1), ("null_r", -1), ("max_steps", -1)]
)
